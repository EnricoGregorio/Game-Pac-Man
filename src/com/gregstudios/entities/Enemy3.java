...
public class Enemy3 extends Entity {
	...
	public Enemy3(int x, int y, int width, int height, double speed, BufferedImage sprite) {
		super(x, y, width, height, speed, sprite);

		rightEnemy3 = new BufferedImage[2];
		leftEnemy3 = new BufferedImage[2];
		upEnemy3 = new BufferedImage[2];
		downEnemy3 = new BufferedImage[2];

		enemyDamaged = new BufferedImage[2];

		for (int i = 0; i < 2; i++) {
			rightEnemy3[i] = Game.spritesheet.getSprite(64 + (i*32), 192, 32, 32);
		}
		for (int i = 0; i < 2; i++) {
			leftEnemy3[i] = Game.spritesheet.getSprite(128 + (i*32), 192, 32, 32);
			
		}
		for (int i = 0; i < 2; i++) {
			downEnemy3[i] = Game.spritesheet.getSprite(128 + (i*32), 224, 32, 32);
		}
		for (int i = 0; i < 2; i++) {
			upEnemy3[i] = Game.spritesheet.getSprite(64 + (i*32), 224, 32, 32);
		}
	...
	public void render(Graphics g) {
		if(lastDir == 1) {
			g.drawImage(rightEnemy3[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}else if(lastDir == -1) {
			g.drawImage(leftEnemy3[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}
		if(lastDir == 2) {
			g.drawImage(upEnemy3[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}else if(lastDir == -2) {
			g.drawImage(downEnemy3[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}if (ghostMode == true){
