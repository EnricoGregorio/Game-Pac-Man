...
public class Enemy2 extends Entity {
    ...
    public Enemy2(int x, int y, int width, int height, double speed, BufferedImage sprite) {
		...
		upEnemy2 = new BufferedImage[2];
		downEnemy2 = new BufferedImage[2];

		enemyDamaged = new BufferedImage[2];

		for (int i = 0; i < 2; i++) {
			rightEnemy2[i] = Game.spritesheet.getSprite(64 + (i*32), 128, 32, 32);
		}
		for (int i = 0; i < 2; i++) {
			leftEnemy2[i] = Game.spritesheet.getSprite(128 + (i*32), 128, 32, 32);
			
		}
		for (int i = 0; i < 2; i++) {
			downEnemy2[i] = Game.spritesheet.getSprite(128 + (i*32), 160, 32, 32);
		}
		for (int i = 0; i < 2; i++) {
			upEnemy2[i] = Game.spritesheet.getSprite(64 + (i*32), 160, 32, 32);
		}
    ...
    public void render(Graphics g) {
        if(lastDir == 1) {
			g.drawImage(rightEnemy2[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}else if(lastDir == -1) {
			g.drawImage(leftEnemy2[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}
		if(lastDir == 2) {
			g.drawImage(upEnemy2[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}else if(lastDir == -2) {
			g.drawImage(downEnemy2[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}if (ghostMode == true){
